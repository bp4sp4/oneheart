...
export async function POST(req: Request) {
  try {
    ...
    logger.log("Incoming payment confirmation request:", { paymentKey, orderId, amount });
    ...
    const { data: orderData, error: orderError } = await supabase
      ...
      .maybeSingle();
    if (orderError) {
      logger.error("DB lookup error:", orderError);
      return NextResponse.json({ message: "DB 조회 오류", detail: orderError.message }, { status: 500 });
    }
    if (orderData && orderData.status === "PAID") {
      logger.log("Order already paid:", { orderId, status: orderData.status });
    ...
    if (!TOSS_API_KEY) {
      logger.error("TOSS_API_KEY not configured.");
    ...
    logger.log("Toss API response:", json);

    if (!res.ok) {
      logger.error("Toss API error response:", json);
    ...
    if (json.status === "DONE") {
      try {
        ...
        logger.error(`Database update failed for orderId ${orderId}:`, dbError);
      }
      // Return a clear success response only when status is DONE
      logger.log("Final success response (Toss DONE):", { ...json, success: true });
    ...
    logger.log("Returning Toss response for non-DONE status:", json);
    return NextResponse.json(json, { status: res.status });
  } catch (err: any) {
    logger.error("/api/toss/confirm error:", err);
    logger.error("/api/toss/confirm error (full object):", JSON.stringify(err, Object.getOwnPropertyNames(err)));
